...
The on-page monitoring panel contains branding, a state-specific explanation, and a semantic Open/Close Picture-in-Picture button, sized 15rem by 10rem, vertically centered and inset 5rem from the right viewport edge. Its static positioning and appearance live in `monitoring-control.css`. The explanation changes per state (not monitoring, opening, monitoring, question active, question ended, unsupported, failed) and carries a `data-state` hook for styling. Failure copy is a normalized category only; it never surfaces raw errors, routes, question content, or identifiers. On viewports at or below 21.5rem wide, the right inset becomes 0.75rem to keep it reachable. Viewport maximum dimensions constrain it on very small screens.

The on-page action labels describe opening/closing PiP rather than starting/stopping route detection: route evaluation already runs while the page is loaded, but visible alerts still require an open, user-started PiP.
