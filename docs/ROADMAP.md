...
The Phase 1 monitoring migration is explicitly owner-authorized. The initial migration added no permissions; the owner-requested pulse preference now adds storage for one local boolean (D014). The exact student-site content match remains. Desktop Chrome 116+ is required. PiP opens from a user click, remains at one requested size, and closes with its opener. No separate-tab/native notification fallback exists. The toolbar popup includes the pulse preference and opens an extension-owned tester tab so PiP states can be exercised without iClicker; it is tooling and does not satisfy the pending production toolbar/settings phase. The old investigation spike is not a prerequisite. [Detection strategy](DETECTION_STRATEGY.md) records route evidence and remaining limitations.
