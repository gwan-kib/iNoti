...
| 2 | Production toolbar/settings and persistent preferences; page monitoring control is already in Phase 1 | Alert pulse preference implemented; broader settings pending |
| 3 | Notification click-to-focus and optional sound with validated browser/OS behavior | Go to Question implemented; browser focus checks and sound pending |
...
The Phase 1 monitoring migration is explicitly owner-authorized. The initial migration added no permissions; the owner-requested pulse preference now adds storage for one local boolean (D014). The exact student-site content match remains. Desktop Chrome 123+ is required. PiP opens from a user click, remains at one requested size, and closes with its opener. No separate-tab/native notification fallback exists. The toolbar popup includes the pulse preference and opens an extension-owned tester tab so PiP states can be exercised without iClicker; it is tooling and does not satisfy the pending production toolbar/settings phase. The old investigation spike is not a prerequisite. [Detection strategy](DETECTION_STRATEGY.md) records route evidence and remaining limitations.
