...
Vite builds an unpacked MV3 extension into `dist/`: the manifest/icon, content and worker bundles, a toolbar popup with the saved alert-animation and sound preferences, a development-test page, and the offscreen audio document. Five standalone IIFE builds run in sequence; the content build clears output and copies the whole `assets/sounds/` directory, later builds preserve it, and the popup/tester/offscreen builds copy their local HTML/CSS. The production PiP DOM/styles remain bundled into content.js; the dev tester reuses the same PiP controller/view code in its own bundle. Generated output and dependencies are ignored by Git. The manifest requires desktop Chrome 123 for Document PiP opener focusing, with runtime feature detection; this minimum is not inferred from ES2022.
...
Join a class after the page refresh: initial `UNSUPPORTED` is expected before SPA initialization. Look for worker `webNavigation history update observed` or `webNavigation fragment update observed`, forwarding/delivery logs, and content `navigation update received` with source `webNavigation` or `hashchange`. `UNSUPPORTED` to `WAITING` sets the baseline; a later same-class `QUESTION_ACTIVE` should log `new question accepted` once, request sound once, and (if open) change the PiP to its alert state once. Check the worker for `sound playback requested` or `sound disabled` and the offscreen document for playback. Injection was verified in the owner's Chrome test; this updated end-to-end flow still needs live verification.
