...
import { PULSE_KEY, watchPulsePreference, type PreferenceStorage } from '../src/shared/alert-preference';
...
function fixture() {
  const storage = {
    local: { get: vi.fn<PreferenceStorage['local']['get']>().mockResolvedValue({}), set: vi.fn<PreferenceStorage['local']['set']>().mockResolvedValue() },
    onChanged: { addListener: vi.fn(), removeListener: vi.fn() },
  };
  const changed = vi.fn();
  const emit = (value: unknown, area = 'local') => {
    storage.onChanged.addListener.mock.calls[0]![0]({ [PULSE_KEY]: { newValue: value } }, area);
  };
  return { storage, changed, emit };
}

it.each([undefined, true, false, 'false'])('loads preference %s with boolean-only defaults', async value => {
  const f = fixture();
  f.storage.local.get.mockResolvedValue({ [PULSE_KEY]: value });
  const stop = watchPulsePreference(f.changed, f.storage);
  await vi.waitFor(() => expect(f.changed).toHaveBeenCalledWith(value === false ? false : true));
  stop();
  expect(f.storage.onChanged.removeListener).toHaveBeenCalledWith(f.storage.onChanged.addListener.mock.calls[0]![0]);
});

it('keeps live local changes ahead of stale reads and ignores updates after cleanup', async () => {
  const f = fixture();
  let resolve!: (value: Record<string, unknown>) => void;
  f.storage.local.get.mockReturnValue(new Promise(done => { resolve = done; }));
  const stop = watchPulsePreference(f.changed, f.storage);
  f.emit(false, 'sync');
...
it('saves the popup toggle and restores its saved value on write failure', async () => {
  ...
  await bindPulseSetting(input as unknown as HTMLInputElement, status as unknown as HTMLElement, f.storage);
