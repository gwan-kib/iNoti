...
describe("route parsing", () => {
  it.each([
    [base, "WAITING"],
    [`${base}/poll`, "QUESTION_ACTIVE"],
    [`${base}/question/${question}`, "QUESTION_CLOSED"],
    [`${base}/quiz/${question}`, "UNSUPPORTED"],
    ["#/home", "UNSUPPORTED"],
    ["#/class/not-a-uuid/poll", "UNSUPPORTED"],
    ["#/class/", "UNSUPPORTED"],
    [`${base}/question/not-a-uuid`, "UNSUPPORTED"],
    [`${base}/poll/extra`, "UNSUPPORTED"],
    [`${base}/poll?extra=true`, "UNSUPPORTED"],
    [`${base}/poll/`, "UNSUPPORTED"],
    ["", "UNSUPPORTED"],
...
describe("transition decisions", () => {
  for (const previous of routes) {
    for (const next of routes) {
      it(`${previous} -> ${next}`, () => {
        const expected =
          (previous === base || previous.includes("/question/")) &&
          next === `${base}/poll`;
        expect(isNewPoll(parseRoute(previous), parseRoute(next))).toBe(
          expected,
        );
      });
    }
  ...
  it("does not infer a poll when changing classes", () => {
    expect(
      isNewPoll(parseRoute(base), parseRoute(`#/class/${question}/poll`)),
    ).toBe(false);
  });
});
