...
it.each([true, false])('opens settings independently of the PiP toggle and reports failures: %s', async (ok) => {
  ...
  const settings = document.elements.find(element => element.className === 'monitoring-settings')!;
  const status = document.elements.find(element => element.className === 'monitoring-settings-status')!;
  settings.dispatchEvent(new Event('click'));
  expect(settings.disabled).toBe(true);
  await vi.waitFor(() => expect(settings.disabled).toBe(false));
  expect(open).toHaveBeenCalledOnce();
  expect(toggle).not.toHaveBeenCalled();
  expect(status.textContent).toBe(ok ? '' : "Couldn't open settings, use toolbar icon.");
});
...
it('sends only the settings action and checks the acknowledgement', async () => {
  ...
  expect(sendMessage).toHaveBeenCalledExactlyOnceWith({ type: 'OPEN_SETTINGS' });
...
it('dismisses a settings error and allows a later failure to appear again', async () => {
  ...
  const notice = document.elements.find(element => element.className === 'monitoring-settings-notice')!;
  const dismiss = document.elements.find(element => element.className === 'monitoring-settings-dismiss')!;
  const status = document.elements.find(element => element.className === 'monitoring-settings-status')!;
