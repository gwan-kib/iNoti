...
async function start(hash: string, supported = true) {
  ...
  const page = Object.assign(new EventTarget(), {
    location: { hash },
    documentPictureInPicture: supported ? { requestWindow } : undefined,
  });
...
it("opens the window synchronously once per user action, suppressing duplicate pending clicks", async () => {
  ...
  // While the window is open the panel has no toggle button at all.
...
it.each(["worker-first", "hashchange-first"])("suppresses duplicate source reports: %s", async (order) => {
  ...
  const app = await start(base);
  await app.open();
  for (const source of order === "worker-first" ? [app.navigation, app.navigate] : [app.navigate, app.navigation])
    source(`${base}/poll`);
...
it("handles unavailable Document PiP without changing iClicker or opening a fallback", async () => {
  const app = await start(base, false);
  await app.open();
...
it.each([
  null,
  { type: "NEW_POLL", detectedAt: 0 },
  { type: "NAVIGATION_CHANGED", hash: 7 },
...
it.each([{ id: "other-extension" }, { id: "test-extension", tab: { id: 1 } }])(
  "rejects untrusted navigation sender %j",
  async (sender) => {
    const app = await start(base);
    await app.open();
