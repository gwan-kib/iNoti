...
it('renders PiP icons with the rounded symbol class', () => {
  ...
  expect(time.children[1]!.className).toBe('material-symbols-rounded clock-icon');
  expect(elapsed.children[1]!.className).toBe('material-symbols-rounded timer-icon');
  expect(document.head.children[0]!.attributes.get('href')).toContain('family=Material+Symbols+Rounded');
});
