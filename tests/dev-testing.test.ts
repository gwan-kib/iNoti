...
it('simulates idle from active and ended, and keeps the pulse override across PiP reopen', async () => {
  ...
  const ids = ['pip-status', 'event-log', 'preview', 'pulse-alerts', 'open-pip', 'idle', 'question', 'ended', 'stop', 'clear-log', 'panel-preview', 'panel-unmonitored', 'panel-idle', 'panel-opening', 'panel-unsupported', 'panel-failed'];
  ...
  click('panel-idle');
  // While PiP is open the toggle is removed; the explanation stays visible.
