...
it('simulates idle from active and ended, and keeps the pulse override across PiP reopen', async () => {
  ...
  expect(panelLabel()).toBe('Open Picture-in-Picture');
  expect(panelExplanation().textContent).toBe('Keep this iClicker page open so iNoti can detect new questions.');
  expect(panelExplanation().attributes.get('data-state')).toBe('unmonitored');
  click('panel-idle');
  expect(panelLabel()).toBe('Close Picture-in-Picture');
  expect(panelExplanation().textContent).toBe('Monitoring is on. Keep this iClicker page open for new-question alerts.');
  expect(panelExplanation().attributes.get('data-state')).toBe('idle');
  click('panel-question');
  expect(panelExplanation().textContent).toBe('A new question is active in the Picture-in-Picture window.');
  expect(panelExplanation().attributes.get('data-state')).toBe('question-active');
  click('panel-opening');
  expect(panelLabel()).toBe('Opening Picture-in-Picture...');
  expect(panelExplanation().textContent).toBe('Opening the Picture-in-Picture window…');
  click('panel-unsupported');
  expect(panelLabel()).toBe('Document PiP unavailable');
  expect(panelExplanation().textContent).toBe('Picture-in-Picture needs desktop Chrome 123 or newer.');
  expect(panelExplanation().attributes.get('data-state')).toBe('unsupported');
  click('panel-failed');
  expect(panelLabel()).toBe('PiP failed - Try again');
  expect(panelExplanation().textContent).toBe('The Picture-in-Picture window could not open. Click the button to try again.');
