...
const testerSender = {
  id: "test-extension",
  url: "chrome-extension://test-extension/dev-testing/index.html",
};
...
const navigation = {
  ...
  url: `https://student.iclicker.com/${hash}`,
};

it.each(["history", "fragment"])(
  "forwards filtered %s updates to the originating top-frame document",
  async (source) => {
    const register = source === "history" ? historyListener : fragmentListener;
    expect(register).toHaveBeenCalledTimes(1);
    expect(register.mock.calls[0]?.[1]).toEqual({
      url: [{ hostEquals: "student.iclicker.com" }],
    });
    register.mock.calls[0]?.[0](navigation);
    await Promise.resolve();
    expect(tabsSendMessage).toHaveBeenCalledExactlyOnceWith(
      7,
      { type: "NAVIGATION_CHANGED", hash },
      { frameId: 0, documentId: "synthetic-document" },
    );
  },
);

it.each([
  { ...navigation, frameId: 1 },
  { ...navigation, tabId: -1 },
...
it("forwards an empty unsupported marker instead of unknown route content", () => {
  historyListener.mock.calls[0]?.[0]({
    ...navigation,
    url: "https://student.iclicker.com/#/quiz/private-data",
  });
  expect(tabsSendMessage.mock.calls[0]?.[1]).toEqual({
...
it("logs forwarding errors without leaking URLs or retrying", async () => {
  ...
  tabsSendMessage.mockRejectedValue(
    new Error(`Receiving end does not exist: ${navigation.url}`),
  );
  ...
  expect(JSON.stringify(log.mock.calls)).toContain(
    "failed to forward navigation update",
  );
...
it("reuses an existing offscreen document instead of creating another", async () => {
  getContexts.mockResolvedValue([
    { documentUrl: getURL("offscreen/offscreen.html") },
  ]);
  messageListener()({ type: "NEW_QUESTION_DETECTED" }, contentSender);
...
it("ignores malformed and untrusted preview requests", () => {
  ...
  expect(
    messageListener()({ type: "PREVIEW_SOUND" }, { id: "other-extension" }),
  ).toBe(false);
  expect(storageGet).not.toHaveBeenCalled();
  expect(createDocument).not.toHaveBeenCalled();
  expect(runtimeSendMessage).not.toHaveBeenCalled();
...
const settingsSender = {
  id: "test-extension",
  frameId: 0,
  url: "https://student.iclicker.com/",
  tab: { id: 7, windowId: 3 },
};
...
it.each([
  ...
  expect(messageListener()({ type: "OPEN_SETTINGS" }, sender, vi.fn())).toBe(
    false,
  );
