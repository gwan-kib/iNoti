...
import { SOUND_ENABLED_KEY, SELECTED_SOUND_KEY, soundEnabled } from '../shared/sound-preference';
...
async function hasOffscreenDocument(): Promise<boolean> {
  ...
  return contexts.some((context) => context.documentUrl === chrome.runtime.getURL(OFFSCREEN_PATH));
}
...
async function ensureOffscreenDocument(): Promise<void> {
  ...
  creatingOffscreen = chrome.offscreen.createDocument({
    ...
    justification: 'Play the configured notification sound when a genuinely new iClicker question is detected. No page content, routes, or identifiers are involved.',
  }).finally(() => { creatingOffscreen = undefined; });
  await creatingOffscreen;
}

async function readSoundPreferences(): Promise<{ enabled: boolean; soundId: SoundId } | undefined> {
  try {
    const values = await chrome.storage.local.get([SOUND_ENABLED_KEY, SELECTED_SOUND_KEY]);
    return {
