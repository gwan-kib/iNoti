import {
  ...
  documentPip,
  type PipStatus,
} from "../content/pip-controller";
import { createPipView } from "../content/pip-view";
import {
  createMonitoringControl,
  type MonitoringPanelStatus,
} from "../content/monitoring-control";
...
function statusText(pip: PipStatus, supported: boolean) {
  ...
  if (pip.issue === "failed")
    return "PiP: opening failed — retry from Open PiP";
...
function initDevTester() {
  ...
  const panelState = (): MonitoringPanelStatus => ({
    monitoring,
    pip: pipStatus,
  });
  const renderStatus = () => {
    statusElement.textContent = statusText(pipStatus, Boolean(api));
  };
  ...
  if (!panelDocument)
    throw new Error("Monitoring panel preview document unavailable");
  const panelControl = createMonitoringControl(panelDocument, () =>
    appendLog("monitoring panel toggle clicked"),
  );
  panelControl.show();
  ...
  const controller = createPipController(
    ...
    (next) => {
      ...
      renderStatus();
      panelControl.render(panelState());
      appendLog("PiP state changed", {
        state: next.state,
        opening: next.opening,
        issue: next.issue ?? "none",
      });
    },
    (pipDocument, onAnswered) => {
      ...
      if (pipDocument.defaultView) {
        stopFollowingSize = followPipSize(
          previewFrame,
          pipDocument.defaultView,
        );
      }
      return view;
    },
  ...
  const renderSound = () => {
    ...
    const label =
      SOUND_OPTIONS.find((option) => option.id === soundIdValue)?.label ??
      soundIdValue;
  ...
  void loadSoundPreferences()
    .then((preferences) => {
      if (!preferences) {
        soundStatus.textContent =
          "Extension storage unavailable here; the worker still owns the sound decision.";
        return;
      }
    ...
    .catch(() => {
      soundStatus.textContent = "Could not load the sound preference.";
    });
  ...
  soundChoice.addEventListener("change", () => {
    ...
    renderSound();
    void saveSelectedSoundId(soundIdValue)
      .then(() => {
  ...
  required<HTMLButtonElement>("test-sound").addEventListener("click", () => {
    appendLog("test sound requested", {
      savedEnabled: soundEnabledValue,
  ...
  required<HTMLButtonElement>("panel-unmonitored").addEventListener(
    "click",
    () =>
      showPanelState(
        { monitoring: false, pip: { state: "CLOSED", opening: false } },
        "not monitoring",
      ),
  );
  required<HTMLButtonElement>("panel-monitoring").addEventListener(
    "click",
    () =>
      showPanelState(
        { monitoring: true, pip: { state: "CLOSED", opening: false } },
        "monitoring, window closed",
      ),
  );
  required<HTMLButtonElement>("panel-opening").addEventListener("click", () =>
    showPanelState(
      { monitoring: true, pip: { state: "CLOSED", opening: true } },
      "opening",
    ),
  );
  required<HTMLButtonElement>("panel-unsupported").addEventListener(
    "click",
    () =>
      showPanelState(
        {
          monitoring: true,
          pip: { state: "CLOSED", opening: false, issue: "unsupported" },
        },
        "unsupported",
      ),
  );
  required<HTMLButtonElement>("panel-failed").addEventListener("click", () =>
    showPanelState(
      {
        monitoring: true,
        pip: { state: "CLOSED", opening: false, issue: "failed" },
      },
      "failed",
    ),
  );
...
if (document.readyState === "loading")
  document.addEventListener("DOMContentLoaded", initDevTester, { once: true });
else initDevTester();
