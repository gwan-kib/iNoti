...
function explanationFor(status: MonitoringStatus, active: boolean): string {
  ...
  if (status.state === 'MONITORING_QUESTION_ACTIVE') return 'A new question is active in the Picture-in-Picture window.';
  if (status.state === 'MONITORING_QUESTION_ENDED') return 'The question ended. Monitoring stays on for the next one.';
  if (active) return 'Monitoring is on. Keep this iClicker page open for new-question alerts.';
  return 'Keep this iClicker page open so iNoti can detect new questions.';
}
...
export function createMonitoringControl(document: Document, toggle: () => void) {
  ...
  return {
    ...
    render(status: MonitoringStatus) {
      ...
      label.textContent = status.issue === 'unsupported' ? 'Document PiP unavailable'
        : status.issue === 'failed' ? 'PiP failed - Try again'
        : status.opening ? 'Opening Picture-in-Picture...' : active ? 'Close Picture-in-Picture' : 'Open Picture-in-Picture';
      button.title = status.issue === 'unsupported' ? 'Picture-in-Picture requires desktop Chrome 123+.'
        : status.issue === 'failed' ? 'Could not open Picture-in-Picture. Click to try again.'
          : active ? 'Close on-screen notifications' : 'Open on-screen notifications';
      button.setAttribute('aria-label', status.issue === 'failed' ? 'Could not open Picture-in-Picture. Try again' : label.textContent);
