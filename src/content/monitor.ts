...
function panelStatus(): MonitoringPanelStatus {
  return {
    monitoring: !suspended && previous.state !== "UNSUPPORTED",
    pip: pipStatus,
  };
}
...
function evaluateHash(hash: string, source: "hashchange" | "webNavigation") {
  ...
  log("navigation update received", {
    source,
    previous: previous.state,
    next: next.state,
    eligibleNewPoll: accepted,
  });
  if (
    next.state === "UNSUPPORTED" ||
    (previous.state !== "UNSUPPORTED" && previous.classId !== next.classId)
  ) {
  ...
  else if (next.state === "WAITING" || next.state === "QUESTION_CLOSED")
...
chrome.runtime.onMessage.addListener(
  (message: unknown, sender, sendResponse) => {
    ...
    if (
      ...
      sender.tab ||
      !isNavigationChangedMessage(message)
    ) {
