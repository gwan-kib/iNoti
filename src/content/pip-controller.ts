...
export type PipState =
  "CLOSED" | "OPEN_IDLE" | "QUESTION_ACTIVE" | "QUESTION_ENDED";
...
export function documentPip(page: Window): DocumentPip | undefined {
  const api = (page as Window & { documentPictureInPicture?: DocumentPip })
...
export function createPipController(
  ...
  makeView: (
    document: Document,
    answerQuestion: () => void,
  ) => PipView = createConfiguredPipView,
  rootFontSize: () => number = () =>
  ...
  return {
    ...
    question(detectedAt: number) {
      ...
      if (
        !pip ||
        !view ||
        (status.state !== "OPEN_IDLE" && status.state !== "QUESTION_ENDED")
      )
        return;
      if (pip.closed) {
        close();
