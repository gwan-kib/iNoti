...
export function createConfiguredPipView(
  document: Document,
  answerQuestion: () => void = () => {},
) {
  ...
  const view = createPipView(document, () => window.focus(), answerQuestion);
  const unsubscribe = watchPulsePreference((enabled) =>
    view.setPulseEnabled(enabled),
  );
  document.defaultView?.addEventListener("pagehide", unsubscribe, {
    once: true,
  });
  return view;
}
