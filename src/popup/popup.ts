...
export function bindPulseSetting(
  input: HTMLInputElement,
  status: HTMLElement,
  storage: PreferenceStorage,
) {
...
export function bindSoundSetting(
  input: HTMLInputElement,
  status: HTMLElement,
  storage: PreferenceStorage,
) {
  return bindBooleanSetting(
    input,
    status,
    storage,
    SOUND_ENABLED_KEY,
...
export async function bindSoundChoice(
  ...
  try {
    saved = resolveSoundId(
      (await storage.local.get(SELECTED_SOUND_KEY))[SELECTED_SOUND_KEY],
    );
    select.value = saved;
...
function initPopup() {
  ...
  if (pulseInput && status && storage)
    void bindPulseSetting(pulseInput, status, storage);
  const soundInput = document.querySelector<HTMLInputElement>("#sound-alerts");
  if (soundInput && status && storage)
  ...
  if (testSound)
    testSound.addEventListener("click", () => requestSoundPreview());
  bindDevTestingButton(
    document.querySelector<HTMLButtonElement>("#open-dev-tester"),
    DEV_TESTING_ENABLED,
    () => {
      openDevTestingTab(
