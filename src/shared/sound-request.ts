...
export function requestNewQuestionSound(
  ...
  send(
    sender,
    { type: "NEW_QUESTION_DETECTED" } satisfies NewQuestionDetectedMessage,
    log,
  );
}
...
export function requestSoundPreview(
  sender: MessageSender = defaultSender,
  log = logger("content"),
) {
