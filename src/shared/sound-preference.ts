...
export async function loadSoundPreferences(storage: PreferenceStorage | undefined = preferenceStorage()): Promise<SoundPreferences | undefined> {
  if (!storage) return undefined;
  return soundPreferences(await storage.local.get([SOUND_ENABLED_KEY, SELECTED_SOUND_KEY]));
}

export async function saveSoundEnabled(enabled: boolean, storage: PreferenceStorage | undefined = preferenceStorage()): Promise<void> {
  await storage?.local.set({ [SOUND_ENABLED_KEY]: enabled });
}

export async function saveSelectedSoundId(soundId: SoundId, storage: PreferenceStorage | undefined = preferenceStorage()): Promise<void> {
